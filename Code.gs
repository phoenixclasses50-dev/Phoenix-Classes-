...
function doGet(e) {
  try {
    ...
    if (action === 'save' && e.parameter.data) {
...
function doPost(e) {
  try {
    ...
    if (data.action === 'save') {
      ...
      return makeResponse({status:'ok', message:'Saved!'}, null);
    }
    return makeResponse({status:'ok', data:loadData(ss)}, null);
  } catch(err) {
    return makeResponse({status:'error', message:err.message}, null);
  }
}
